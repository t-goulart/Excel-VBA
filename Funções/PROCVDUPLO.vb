Public Function PROCVDUPLO(Valores_Procurados, Primeira_Coluna As Range, Segunda_Coluna As Range, Matriz_Tabela As Range, Coluna_Resultado)
'____ Funciona como INDICE + CORRESP limitado a 2 valores procurados
'Valores_Procurados: Recebe dois valores concatenados formando um valor único
'Primeira_Coluna: Coluna que contém a primeira palavra das duas contidas dentro de Valores_Procurados
'Segunda_Coluna: Coluna que contém a segunda palavra das duas contidas dentro de Valores_Procurados
...
'Inicia na primeira linha do Range de cada coluna (Primeira_Coluna e Segunda_Coluna) | 'Ñ é a primeira linha da planilha
Linha = 1
...
For Each Elemento In Primeira_Coluna
    ...
    If VarType(Primeira_Coluna.Rows(Linha)) = vbDate Then
        ...
        Concatena_Colunas = WorksheetFunction.Concat(CLng(Primeira_Coluna.Rows(Linha)), Trim(Segunda_Coluna.Rows(Linha)))
    ...
    ElseIf VarType(Segunda_Coluna.Rows(Linha)) = vbDate Then
        ...
        Concatena_Colunas = WorksheetFunction.Concat(Trim(Primeira_Coluna.Rows(Linha)), CLng(Segunda_Coluna.Rows(Linha)))
    ...
    Else
        ...
        Concatena_Colunas = WorksheetFunction.Concat(Trim(Primeira_Coluna.Rows(Linha)), Trim(Segunda_Coluna.Rows(Linha)))
    End If
    ...
    'Se os valores informados são iguais a concatenação da linha atual e correspondente as duas colunas informadas
    If Valores_Procurados = Concatena_Colunas Then
        ...
        QX_PROCVDUPLO = Matriz_Tabela.Cells(Linha, Coluna_Resultado)
        'Sai do laço
        Exit For
    End If
